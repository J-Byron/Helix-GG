...
router.get('/:region/:summonerName/:queue', (req, res) => {
    ...
    let summonerData = {
        queue: (queueType === 420) ? 'Ranked' : 'Normal',
    ...
    axios.get(`https://na1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${summonerName}?api_key=${API_KEY}`).then(response => {
        ...
        axios.get(`https://na1.api.riotgames.com/lol/match/v4/matchlists/by-account/${summonerIDs.accountId}?queue=${queueType}&endIndex=10&beginIndex=0&api_key=${API_KEY}`)
            .then(response => {
                ...
                */
                ...
                (async () => {
                    ...
                    // Array of match requests to be made asynchonously
                    const matchCalls = [];

                    // Push each match into match call as a request
                    for (let match of response.data.matches) {
                        const matchCall = axios.get(`https://na1.api.riotgames.com/lol/match/v4/matches/${match.gameId}?api_key=${API_KEY}`);
                        matchCalls.push(matchCall);
                    }

                    // Asynchonously promise and execute all requests
                    Promise.all(matchCalls).then((responses) => {
                        ...
                        responses.forEach((match, i) => {
                            ...
                            for (let participant of matchData.participantIdentities) {
                                if (summonerName.toUpperCase() === participant.player.summonerName.toUpperCase()) {
                                    ...
                                    queriedMatchParticipant = matchData.participants[participant.participantId - 1];
                                    break;
                                }
                            }
                            ...
                            matchInformation.matchParticipants = matchData.participantIdentities.filter(participant => {
                                ...
                                const player = matchData.participants[participant.participantId - 1];
                                return {
                        ...
                        */
                        ...
                        console.log(`All matches succesfully loaded :) `);
                    }
                    ).catch(error => {
